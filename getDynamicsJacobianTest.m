function A = getDynamicsJacobianTest(mu, uKF, dt)
    % Estimated states:
    vg  = mu(3);
    chi = mu(4);
    wn  = mu(5);
    ...
    % estimated states [pn, pe, vg, chi, wnTot, weTot, wns, wes, wnt, wet]
 ...
 %% Linearized Dryden Model (MIL-F-8785C)   
    ...
    A(7, 5) = -1 * (1/Lu) * (delVa_delUs) * wnTurb;
    A(7, 6) = -1 * (1/Lu) * (delVa_delVs) * wnTurb;
    A(8, 5) = -1 * (1/Lv) * (delVa_delUs) * weTurb;
    A(8, 6) = -1 * (1/Lv) * (delVa_delVs) * weTurb;
    A(7, 7) = -1 * dt*(va / Lu);
    A(8, 8) = -1 * dt*(va / Lv);
