...
%     v_t_ned_mps = v_t_ned_kts * kts2mps; 
    ...
    u_t_ned_mps = (1 - va * dt / Lu) * wnTurb + realsqrt(2 * va * dt / Lu) * sigmaU * wnNoise;
    v_t_ned_mps = (1 - va * dt / Lv) * weTurb + realsqrt(2 * va * dt / Lv) * sigmaV * weNoise;

    
    mu_kp = [mu(1) + dt * vg*cos(chi);
             mu(2) + dt * vg*sin(chi);
             mu(3) + dt * ((va*cos(psi)+wnStc + wnTurb)*(-va*psiDot*sin(psi)) + (va*sin(psi) + weStc + weTurb)*(va*psiDot*cos(psi)))/vg;
