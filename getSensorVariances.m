...
deg2rad = pi/180;
    ...
    switch sensorType
        case 'low' 
            % Low-tier (RC)
            var.airspeed_mps        = 1^2;              % SDP3x (https://www.sensirion.com/en/flow-sensors/differential-pressure-sensors/worlds-smallest-differential-pressure-sensor/)
            var.attitude_rad        = 0.5 * deg2rad;    % VectorNav-100 (Heading accuracy is 2deg RMS)
            var.pqr_radps           = 0.5^2 * deg2rad;  % VectorNav-100
